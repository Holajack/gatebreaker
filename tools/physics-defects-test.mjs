...
//
//   node tools/physics-defects-test.mjs
...
const r = await evalGame(page, (g) => {
  ...
  out.badNormalGuard = {
    nx: body.nx, ny: body.ny, nz: body.nz,
  ...
  out.solverCost = { callsPer200: `${elapsed.toFixed(2)}ms`, perCallUs: +((elapsed / 200) * 1000).toFixed(1) };

  // D5: zero friction must not collapse the bisection to ~nothing.
  const prevF = body.friction;
  body.friction = 0;
...
console.log('report:', writeReport('physics-defects-test', { result: r, errors }));
await browser.close();
await server.stop();
