...
const result = await page.evaluate(({ gateIndex, simSeconds, forceLevel }) => {
  ...
  const inp = g.input;
  const steps = Math.floor(simSeconds * 60);
  let bossSeen = false;
  let maxEnemies = 0;
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (e) {
      ...
      if (d > 2.2) { inp.move.x = dx / d; inp.move.y = -dz / d; }
      else {
